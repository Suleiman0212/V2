...
ScriptVmStatus ScriptVm::get_status() const { return status; }
  ...
  if (params.size() != fn.param_types.size()) {
    throw std::runtime_error(
        std::format("function expects {} parameters, but {} were passed",
                    fn.param_types.size(), params.size()));
  }
...
void ScriptVm::yield() {
  if (status != ScriptVmStatus::Running)
    throw std::runtime_error("vm isn't in running status");
  status = ScriptVmStatus::Yielded;
}

void ScriptVm::resume() {
  if (status != ScriptVmStatus::Yielded)
    throw std::runtime_error("vm isn't in yielded status");
  exec();
}
  ...
  while (!exec_stack.empty()) {
    ...
    case AstNodeType::While: {
      ...
      if (eval(while_node->cond)) {
        exec_stack.push_back(
            &node); // make sure to reevaluate the condition later
...
ScriptCell ScriptVm::eval(const AstNodePtr &node) {
  ...
  case AstNodeType::DesStruct: {
    ...
    const auto &factory_fn =
        script.registry.native_fns[des_struct->factory_idx];
  ...
  case AstNodeType::Binary: {
    ...
    switch (binary->op) {
    case BinaryOp::Add:
      return eval(binary->lhs) + eval(binary->rhs);
    case BinaryOp::Sub:
      return eval(binary->lhs) - eval(binary->rhs);
    case BinaryOp::Mul:
      return eval(binary->lhs) * eval(binary->rhs);
    case BinaryOp::Div:
      return eval(binary->lhs) / eval(binary->rhs);
    case BinaryOp::Mod:
      return std::fmod(eval(binary->lhs), eval(binary->rhs));
  ...
  case AstNodeType::NativeCall: {
    ...
    std::vector<ScriptCell> params;
    for (const auto &param : native_call->params)
      params.push_back(eval(param));
