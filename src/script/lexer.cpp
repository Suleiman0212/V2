...
again:
  ...
  case '{':
    return emit(TokenType::LeftBrace);
  case '}':
    return emit(TokenType::RightBrace);
  case '!':
    if (match('='))
      return emit(TokenType::NotEq);
    return emit(TokenType::Not);
  case '+':
    if (match('='))
      return emit(TokenType::PlusAssign);
    return emit(TokenType::Plus);
  case '-':
    if (match('='))
      return emit(TokenType::MinusAssign);
    return emit(TokenType::Minus);
  case '*':
    if (match('='))
      return emit(TokenType::StarAssign);
  ...
  case '%':
    if (match('='))
      return emit(TokenType::PercentAssign);
  ...
  case '<':
    if (match('='))
      return emit(TokenType::LessEq);
    return emit(TokenType::Less);
  case '>':
    if (match('='))
      return emit(TokenType::GreaterEq);
    return emit(TokenType::Greater);
  case '&':
    ch = next();
    if (ch == '&')
      return emit(TokenType::AndAnd);
    throw_error(std::format("expected '&' after '&', got '{}'", ch));
    goto again;
  case '|':
    ch = next();
    if (ch == '|')
      return emit(TokenType::PipePipe);
    throw_error(std::format("expected '|' after '|', got '{}'", ch));
    goto again;
  case ',':
  ...
  case '"':
    if (auto token = read_string())
      return token;
    goto again;
  default: {
    if (isdigit(ch)) {
      if (auto token = read_number())
        return token;
...
std::optional<Token> Lexer::read_number() {
  ...
  double value;
  auto conv_result =
      std::from_chars(source.data() + token_pos, source.data() + pos, value);
...
bool Lexer::has_error() { return error_flag; }
